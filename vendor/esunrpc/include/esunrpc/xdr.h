...
#ifndef _ESUNRPC_XDR_H_
#define _ESUNRPC_XDR_H_
 ...
 */
__be32 *esunrpc_xdr_encode_opaque_fixed(__be32 *p, const void *ptr, unsigned int len);
__be32 *esunrpc_xdr_encode_opaque(__be32 *p, const void *ptr, unsigned int len);
__be32 *esunrpc_xdr_encode_string(__be32 *p, const char *s);
__be32 *esunrpc_xdr_encode_netobj(__be32 *p, const struct xdr_netobj *);

void	esunrpc_xdr_inline_pages(struct xdr_buf *, unsigned int,
			 struct page **, unsigned int, unsigned int);
void	esunrpc_xdr_terminate_string(const struct xdr_buf *, const u32);
...
{
	return esunrpc_xdr_encode_opaque(p, s, len);
}
 ...
 */
extern void esunrpc_xdr_buf_from_iov(const struct kvec *, struct xdr_buf *);
extern int esunrpc_xdr_buf_subsegment(const struct xdr_buf *, struct xdr_buf *, unsigned int, unsigned int);
extern void esunrpc_xdr_buf_trim(struct xdr_buf *, unsigned int);
extern int esunrpc_read_bytes_from_xdr_buf(const struct xdr_buf *, unsigned int, void *, unsigned int);
extern int esunrpc_write_bytes_to_xdr_buf(const struct xdr_buf *, unsigned int, void *, unsigned int);

extern int esunrpc_xdr_encode_word(const struct xdr_buf *, unsigned int, u32);
extern int esunrpc_xdr_decode_word(const struct xdr_buf *, unsigned int, u32 *);
...
extern int esunrpc_xdr_decode_array2(const struct xdr_buf *buf, unsigned int base,
			     struct xdr_array2_desc *desc);
extern int esunrpc_xdr_encode_array2(const struct xdr_buf *buf, unsigned int base,
			     struct xdr_array2_desc *desc);
extern void esunrpc__copy_from_pages(char *p, struct page **pages, size_t pgbase,
			     size_t len);
...
extern void esunrpc_xdr_init_encode(struct xdr_stream *xdr, struct xdr_buf *buf,
			    __be32 *p, struct rpc_rqst *rqst);
void esunrpc_xdr_init_encode_pages(struct xdr_stream *xdr, struct xdr_buf *buf);
extern __be32 *esunrpc_xdr_reserve_space(struct xdr_stream *xdr, size_t nbytes);
extern int esunrpc_xdr_reserve_space_vec(struct xdr_stream *xdr, size_t nbytes);
extern void esunrpc___xdr_commit_encode(struct xdr_stream *xdr);
extern void esunrpc_xdr_truncate_encode(struct xdr_stream *xdr, size_t len);
...
extern void esunrpc_xdr_write_pages(struct xdr_stream *xdr, struct page **pages,
...
extern void esunrpc_xdr_init_decode(struct xdr_stream *xdr, struct xdr_buf *buf,
			    __be32 *p, struct rpc_rqst *rqst);
extern void esunrpc_xdr_init_decode_pages(struct xdr_stream *xdr, struct xdr_buf *buf,
		struct page **pages, unsigned int len);
extern void esunrpc_xdr_finish_decode(struct xdr_stream *xdr);
extern __be32 *esunrpc_xdr_inline_decode(struct xdr_stream *xdr, size_t nbytes);
extern unsigned int esunrpc_xdr_read_pages(struct xdr_stream *xdr, unsigned int len);
extern void esunrpc_xdr_enter_page(struct xdr_stream *xdr, unsigned int len);
extern int esunrpc_xdr_process_buf(const struct xdr_buf *buf, unsigned int offset, unsigned int len, int (*actor)(struct scatterlist *, void *), void *data);
extern void esunrpc_xdr_set_pagelen(struct xdr_stream *, unsigned int len);
extern bool esunrpc_xdr_stream_subsegment(struct xdr_stream *xdr, struct xdr_buf *subbuf,
				  unsigned int len);
extern unsigned int esunrpc_xdr_stream_move_subsegment(struct xdr_stream *xdr, unsigned int offset,
					       unsigned int target, unsigned int length);
extern unsigned int esunrpc_xdr_stream_zero(struct xdr_stream *xdr, unsigned int offset,
				    unsigned int length);
 ...
 * If an esunrpc_xdr_inline_decode() call spans across page boundaries, then
...
static inline ssize_t xdr_stream_encode_item_present(struct xdr_stream *xdr)
{
	...
	__be32 *p = esunrpc_xdr_reserve_space(xdr, len);
...
xdr_stream_encode_opaque_inline(struct xdr_stream *xdr, void **ptr, size_t len)
{
	...
	__be32 *p = esunrpc_xdr_reserve_space(xdr, count);

	if (unlikely(!p)) {
		*ptr = NULL;
		return -EMSGSIZE;
	}
	esunrpc_xdr_encode_opaque(p, NULL, len);
	*ptr = ++p;
	return count;
}
...
xdr_stream_encode_opaque_fixed(struct xdr_stream *xdr, const void *ptr, size_t len)
{
	...
	if (unlikely(!p))
		return -EMSGSIZE;
	esunrpc_xdr_encode_opaque_fixed(p, ptr, len);
...
xdr_stream_encode_opaque(struct xdr_stream *xdr, const void *ptr, size_t len)
{
	...
	if (unlikely(!p))
		return -EMSGSIZE;
	esunrpc_xdr_encode_opaque(p, ptr, len);
	return count;
}
		...
		const __u32 *array, size_t array_size)
{
	...
	__be32 *p = esunrpc_xdr_reserve_space(xdr, ret);
...
xdr_stream_decode_bool(struct xdr_stream *xdr, __u32 *ptr)
{
	...
	__be32 *p = esunrpc_xdr_inline_decode(xdr, count);
...
{
	__be32 *p = esunrpc_xdr_inline_decode(xdr, len);
 ...
 * buffer has been destroyed, or even after calling esunrpc_xdr_inline_decode()
...
xdr_stream_decode_opaque_inline(struct xdr_stream *xdr, void **ptr, size_t maxlen)
{
	...
	*ptr = NULL;
	if (unlikely(xdr_stream_decode_u32(xdr, &len) < 0))
		return -EBADMSG;
	if (len != 0) {
		p = esunrpc_xdr_inline_decode(xdr, len);
		if (unlikely(!p))
			return -EBADMSG;
		...
		__u32 *array, size_t array_size)
{
	...
	if (U32_MAX >= SIZE_MAX / sizeof(*p) && len > SIZE_MAX / sizeof(*p))
		return -EBADMSG;
	p = esunrpc_xdr_inline_decode(xdr, len * sizeof(*p));
	if (unlikely(!p))
		return -EBADMSG;
	if (array == NULL)
		return len;
