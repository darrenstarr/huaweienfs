...
/* Flags for esunrpc_rpcauth_lookupcred() */
...
int			esunrpc_rpcauth_register(const struct rpc_authops *);
int			esunrpc_rpcauth_unregister(const struct rpc_authops *);
struct rpc_auth *	esunrpc_rpcauth_create(const struct rpc_auth_create_args *,
				struct rpc_clnt *);
void			rpcauth_release(struct rpc_auth *);
rpc_authflavor_t	esunrpc_rpcauth_get_pseudoflavor(rpc_authflavor_t,
				struct rpcsec_gss_info *);
int			esunrpc_rpcauth_get_gssinfo(rpc_authflavor_t,
				struct rpcsec_gss_info *);
struct rpc_cred *	esunrpc_rpcauth_lookup_credcache(struct rpc_auth *, struct auth_cred *, int, gfp_t);
void			esunrpc_rpcauth_init_cred(struct rpc_cred *, const struct auth_cred *, struct rpc_auth *, const struct rpc_credops *);
struct rpc_cred *	esunrpc_rpcauth_lookupcred(struct rpc_auth *, int);
void			esunrpc_put_rpccred(struct rpc_cred *);
int			rpcauth_marshcred(struct rpc_task *task,
					  struct xdr_stream *xdr);
int			rpcauth_checkverf(struct rpc_task *task,
					  struct xdr_stream *xdr);
int			esunrpc_rpcauth_wrap_req_encode(struct rpc_task *task,
						struct xdr_stream *xdr);
int			rpcauth_wrap_req(struct rpc_task *task,
					 struct xdr_stream *xdr);
int			esunrpc_rpcauth_unwrap_resp_decode(struct rpc_task *task,
						   struct xdr_stream *xdr);
...
int			esunrpc_rpcauth_init_credcache(struct rpc_auth *);
void			esunrpc_rpcauth_destroy_credcache(struct rpc_auth *);
void			rpcauth_clear_credcache(struct rpc_cred_cache *);
char *			esunrpc_rpcauth_stringify_acceptor(struct rpc_cred *);
