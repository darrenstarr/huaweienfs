 ...
 */
void esunrpc_svc_seq_show(struct seq_file *seq, const struct svc_stat *statp)
{
...
EXPORT_SYMBOL_GPL(esunrpc_svc_seq_show);

/**
 * esunrpc_rpc_alloc_iostats - allocate an rpc_iostats structure
 * @clnt: RPC program, version, and xprt
 *
 */
struct rpc_iostats *esunrpc_rpc_alloc_iostats(struct rpc_clnt *clnt)
{
...
EXPORT_SYMBOL_GPL(esunrpc_rpc_alloc_iostats);

/**
 * esunrpc_rpc_free_iostats - release an rpc_iostats structure
 * @stats: doomed rpc_iostats structure
 *
 */
void esunrpc_rpc_free_iostats(struct rpc_iostats *stats)
{
	kfree(stats);
}
EXPORT_SYMBOL_GPL(esunrpc_rpc_free_iostats);

/**
 * esunrpc_rpc_count_iostats_metrics - tally up per-task stats
 * @task: completed rpc_task
 * @op_metrics: stat structure for OP that will accumulate stats from @task
 */
void esunrpc_rpc_count_iostats_metrics(const struct rpc_task *task,
...
EXPORT_SYMBOL_GPL(esunrpc_rpc_count_iostats_metrics);

/**
 * esunrpc_rpc_count_iostats - tally up per-task stats
 * @task: completed rpc_task
 * @stats: array of stat structures
 *
 * Uses the statidx from @task
 */
void esunrpc_rpc_count_iostats(const struct rpc_task *task, struct rpc_iostats *stats)
{
	esunrpc_rpc_count_iostats_metrics(task,
				  &stats[task->tk_msg.rpc_proc->p_statidx]);
}
EXPORT_SYMBOL_GPL(esunrpc_rpc_count_iostats);
...
void esunrpc_rpc_clnt_show_stats(struct seq_file *seq, struct rpc_clnt *clnt)
{
	...
	esunrpc_rpc_clnt_iterate_for_each_xprt(clnt, do_print_stats, seq);
...
EXPORT_SYMBOL_GPL(esunrpc_rpc_clnt_show_stats);
...
esunrpc_rpc_proc_register(struct net *net, struct rpc_stat *statp)
{
	return do_register(net, statp->program->name, statp, &rpc_proc_ops);
}
EXPORT_SYMBOL_GPL(esunrpc_rpc_proc_register);

void
esunrpc_rpc_proc_unregister(struct net *net, const char *name)
{
	struct sunrpc_net *sn;
...
EXPORT_SYMBOL_GPL(esunrpc_rpc_proc_unregister);

struct proc_dir_entry *
esunrpc_svc_proc_register(struct net *net, struct svc_stat *statp, const struct proc_ops *proc_ops)
{
	return do_register(net, statp->program->pg_name, net, proc_ops);
}
EXPORT_SYMBOL_GPL(esunrpc_svc_proc_register);

void
esunrpc_svc_proc_unregister(struct net *net, const char *name)
{
	struct sunrpc_net *sn;
...
EXPORT_SYMBOL_GPL(esunrpc_svc_proc_unregister);
...
int rpc_proc_init(struct net *net)
{
	...
	sn = net_generic(net, esunrpc_sunrpc_net_id);
	sn->proc_net_rpc = proc_mkdir("esunrpc", net->proc_net);
...
void rpc_proc_exit(struct net *net)
{
	...
	remove_proc_entry("esunrpc", net->proc_net);
}
