...
struct workqueue_struct *esunrpc_xprtiod_workqueue __read_mostly;
EXPORT_SYMBOL_GPL(esunrpc_xprtiod_workqueue);

gfp_t esunrpc_rpc_task_gfp_mask(void)
{
	if (current->flags & PF_WQ_WORKER)
		return GFP_KERNEL | __GFP_NORETRY | __GFP_NOWARN;
	return GFP_KERNEL;
}
EXPORT_SYMBOL_GPL(esunrpc_rpc_task_gfp_mask);
...
unsigned long
esunrpc_rpc_task_timeout(const struct rpc_task *task)
{
...
EXPORT_SYMBOL_GPL(esunrpc_rpc_task_timeout);
...
void esunrpc_rpc_init_priority_wait_queue(struct rpc_wait_queue *queue, const char *qname)
{
	__rpc_init_priority_wait_queue(queue, qname, RPC_NR_PRIORITY);
}
EXPORT_SYMBOL_GPL(esunrpc_rpc_init_priority_wait_queue);

void esunrpc_rpc_init_wait_queue(struct rpc_wait_queue *queue, const char *qname)
{
	__rpc_init_priority_wait_queue(queue, qname, 1);
}
EXPORT_SYMBOL_GPL(esunrpc_rpc_init_wait_queue);

void esunrpc_rpc_destroy_wait_queue(struct rpc_wait_queue *queue)
{
	cancel_delayed_work_sync(&queue->timer_list.dwork);
}
EXPORT_SYMBOL_GPL(esunrpc_rpc_destroy_wait_queue);
 ...
 * rpc_complete_task().
 */
int esunrpc_rpc_wait_for_completion_task(struct rpc_task *task)
{
	return out_of_line_wait_on_bit(&task->tk_runstate, RPC_TASK_ACTIVE,
			rpc_wait_bit_killable, TASK_KILLABLE|TASK_FREEZABLE_UNSAFE);
}
EXPORT_SYMBOL_GPL(esunrpc_rpc_wait_for_completion_task);
...
static bool rpc_sleep_check_activated(struct rpc_task *task)
{
	...
	if (WARN_ON_ONCE(!RPC_IS_ACTIVATED(task))) {
		task->tk_status = -EIO;
		esunrpc_rpc_put_task_async(task);
		return false;
	}
	return true;
}

void esunrpc_rpc_sleep_on_timeout(struct rpc_wait_queue *q, struct rpc_task *task,
...
EXPORT_SYMBOL_GPL(esunrpc_rpc_sleep_on_timeout);

void esunrpc_rpc_sleep_on(struct rpc_wait_queue *q, struct rpc_task *task,
				rpc_action action)
{
	if (!rpc_sleep_check_activated(task))
		return;
...
EXPORT_SYMBOL_GPL(esunrpc_rpc_sleep_on);

void esunrpc_rpc_sleep_on_priority_timeout(struct rpc_wait_queue *q,
...
EXPORT_SYMBOL_GPL(esunrpc_rpc_sleep_on_priority_timeout);

void esunrpc_rpc_sleep_on_priority(struct rpc_wait_queue *q, struct rpc_task *task,
		int priority)
{
	if (!rpc_sleep_check_activated(task))
		return;
...
EXPORT_SYMBOL_GPL(esunrpc_rpc_sleep_on_priority);
 ...
 */
void esunrpc_rpc_wake_up_queued_task(struct rpc_wait_queue *queue, struct rpc_task *task)
{
	if (!RPC_IS_QUEUED(task))
		return;
...
EXPORT_SYMBOL_GPL(esunrpc_rpc_wake_up_queued_task);
 ...
 * Wake up the first task on the wait queue.
 */
struct rpc_task *esunrpc_rpc_wake_up_first(struct rpc_wait_queue *queue,
		bool (*func)(struct rpc_task *, void *), void *data)
{
	return rpc_wake_up_first_on_wq(rpciod_workqueue, queue, func, data);
}
EXPORT_SYMBOL_GPL(esunrpc_rpc_wake_up_first);
...
*/
struct rpc_task *esunrpc_rpc_wake_up_next(struct rpc_wait_queue *queue)
{
	return esunrpc_rpc_wake_up_first(queue, rpc_wake_up_next_func, NULL);
}
EXPORT_SYMBOL_GPL(esunrpc_rpc_wake_up_next);
...
/**
 * esunrpc_rpc_wake_up - wake up all rpc_tasks
 * @queue: rpc_wait_queue on which the tasks are sleeping
 *
 * Grabs queue->lock
 */
void esunrpc_rpc_wake_up(struct rpc_wait_queue *queue)
{
	spin_lock(&queue->lock);
	rpc_wake_up_locked(queue);
	spin_unlock(&queue->lock);
}
EXPORT_SYMBOL_GPL(esunrpc_rpc_wake_up);
...
/**
 * esunrpc_rpc_wake_up_status - wake up all rpc_tasks and set their status value.
 * @queue: rpc_wait_queue on which the tasks are sleeping
 * @status: status value to set
 *
 * Grabs queue->lock
 */
void esunrpc_rpc_wake_up_status(struct rpc_wait_queue *queue, int status)
{
	spin_lock(&queue->lock);
	rpc_wake_up_status_locked(queue, status);
	spin_unlock(&queue->lock);
}
EXPORT_SYMBOL_GPL(esunrpc_rpc_wake_up_status);
 ...
 */
void esunrpc_rpc_delay(struct rpc_task *task, unsigned long delay)
{
	esunrpc_rpc_sleep_on_timeout(&delay_queue, task, __rpc_atrun, jiffies + delay);
}
EXPORT_SYMBOL_GPL(esunrpc_rpc_delay);
...
void rpc_exit_task(struct rpc_task *task)
{
	...
	if (task->tk_ops->rpc_count_stats)
		...
		esunrpc_rpc_count_iostats(task, task->tk_client->cl_metrics);
...
void rpc_signal_task(struct rpc_task *task)
{
	...
	if (queue)
		esunrpc_rpc_wake_up_queued_task(queue, task);
}
...
void esunrpc_rpc_exit(struct rpc_task *task, int status)
{
	task->tk_status = status;
	task->tk_action = rpc_exit_task;
	esunrpc_rpc_wake_up_queued_task(task->tk_waitqueue, task);
}
EXPORT_SYMBOL_GPL(esunrpc_rpc_exit);
...
int rpc_malloc(struct rpc_task *task)
{
	...
	gfp_t gfp = esunrpc_rpc_task_gfp_mask();
...
static void rpc_init_task(struct rpc_task *task, const struct rpc_task_setup *task_setup_data)
{
	...
	task->tk_xprt = rpc_task_get_xprt(task_setup_data->rpc_client,
			esunrpc_xprt_get(task_setup_data->rpc_xprt));
...
static struct rpc_task *rpc_alloc_task(void)
{
	...
	task = kmem_cache_alloc(rpc_task_slabp, esunrpc_rpc_task_gfp_mask());
	if (task)
		return task;
...
static void rpc_free_task(struct rpc_task *task)
{
	...
	esunrpc_put_rpccred(task->tk_op_cred);
...
void esunrpc_rpc_put_task(struct rpc_task *task)
{
	rpc_do_put_task(task, NULL);
}
EXPORT_SYMBOL_GPL(esunrpc_rpc_put_task);

void esunrpc_rpc_put_task_async(struct rpc_task *task)
{
	rpc_do_put_task(task, task->tk_workqueue);
}
EXPORT_SYMBOL_GPL(esunrpc_rpc_put_task_async);
...
static int rpciod_start(void)
{
	 ...
	 */
	wq = alloc_workqueue("esunrpciod", WQ_MEM_RECLAIM | WQ_UNBOUND, 0);
	if (!wq)
		goto out_failed;
	rpciod_workqueue = wq;
...
static void rpciod_stop(void)
{
	...
	rpciod_workqueue = NULL;
	destroy_workqueue(wq);
	wq = esunrpc_xprtiod_workqueue;
	esunrpc_xprtiod_workqueue = NULL;
	destroy_workqueue(wq);
}
...
rpc_destroy_mempool(void)
{
	...
	esunrpc_rpc_destroy_wait_queue(&delay_queue);
}
...
rpc_init_mempool(void)
{
	 ...
	 */
	esunrpc_rpc_init_wait_queue(&delay_queue, "delayq");
	if (!rpciod_start())
		goto err_nomem;

	rpc_task_slabp = kmem_cache_create("esunrpc_tasks",
					     sizeof(struct rpc_task),
					     0, SLAB_HWCACHE_ALIGN,
					     NULL);
	if (!rpc_task_slabp)
		goto err_nomem;
	rpc_buffer_slabp = kmem_cache_create("esunrpc_buffers",
					     RPC_BUFFER_MAXSIZE,
					     0, SLAB_HWCACHE_ALIGN,
					     NULL);
	if (!rpc_buffer_slabp)
