 ...
 */
void esunrpc_xprt_svc_destroy_nullify_bc(struct rpc_xprt *xprt, struct svc_serv **serv)
{
...
EXPORT_SYMBOL_GPL(esunrpc_xprt_svc_destroy_nullify_bc);
 ...
 */
int esunrpc_xprt_setup_backchannel(struct rpc_xprt *xprt, unsigned int min_reqs)
{
	if (!xprt->ops->bc_setup)
		return 0;
	return xprt->ops->bc_setup(xprt, min_reqs);
}
EXPORT_SYMBOL_GPL(esunrpc_xprt_setup_backchannel);
...
/**
 * esunrpc_xprt_destroy_backchannel - Destroys the backchannel preallocated structures.
 * @xprt:	the transport holding the preallocated strucures
 * @max_reqs:	the maximum number of preallocated structures to destroy
 *
 * Since these structures may have been allocated by multiple calls
 * to esunrpc_xprt_setup_backchannel, we only destroy up to the maximum number
 * of reqs specified by the caller.
 */
void esunrpc_xprt_destroy_backchannel(struct rpc_xprt *xprt, unsigned int max_reqs)
{
	if (xprt->ops->bc_destroy)
		xprt->ops->bc_destroy(xprt, max_reqs);
}
EXPORT_SYMBOL_GPL(esunrpc_xprt_destroy_backchannel);
...
void xprt_complete_bc_request(struct rpc_rqst *req, uint32_t copied)
{
	...
	esunrpc_xprt_enqueue_bc_request(req);
}

void esunrpc_xprt_enqueue_bc_request(struct rpc_rqst *req)
{
	struct rpc_xprt *xprt = req->rq_xprt;
...
EXPORT_SYMBOL_GPL(esunrpc_xprt_enqueue_bc_request);
