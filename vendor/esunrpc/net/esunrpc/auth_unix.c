					...
					struct auth_cred *acred, int flags)
{
	...
	ret = kmalloc_obj(*ret, esunrpc_rpc_task_gfp_mask());
	if (!ret) {
	...
	esunrpc_rpcauth_init_cred(ret, acred, auth, &unix_credops);
...
unx_marshal(struct rpc_task *task, struct xdr_stream *xdr)
{
	...
	p = esunrpc_xdr_reserve_space(xdr, 3 * sizeof(*p));
	if (!p)
		goto marshal_failed;
	*p++ = rpc_auth_unix;
	cred_len = p++;
	...
	*cred_len = cpu_to_be32((p - cred_len - 1) << 2);
	p = esunrpc_xdr_reserve_space(xdr, (p - gidarr_len - 1) << 2);
	if (!p)
		goto marshal_failed;

	/* Verifier */

	p = esunrpc_xdr_reserve_space(xdr, 2 * sizeof(*p));
	if (!p)
		goto marshal_failed;
	*p++ = rpc_auth_null;
	*p   = xdr_zero;
...
unx_validate(struct rpc_task *task, struct xdr_stream *xdr)
{
	...
	p = esunrpc_xdr_inline_decode(xdr, 2 * sizeof(*p));
	if (!p)
		return -EIO;
	switch (*p++) {
	case rpc_auth_null:
	...
	if (size > RPC_MAX_AUTH_SIZE)
		return -EIO;
	p = esunrpc_xdr_inline_decode(xdr, size);
	if (!p)
		return -EIO;
...
const struct rpc_credops unix_credops = {
	...
	.crwrap_req	= esunrpc_rpcauth_wrap_req_encode,
	.crrefresh	= unx_refresh,
	.crvalidate	= unx_validate,
	.crunwrap_resp	= esunrpc_rpcauth_unwrap_resp_decode,
};
