...
static void rpcb_wake_rpcbind_waiters(struct rpc_xprt *xprt, int status)
{
	...
	esunrpc_rpc_wake_up_status(&xprt->binding, status);
}
...
void rpcb_put_local(struct net *net)
{
	...
	if (shutdown) {
		 ...
		 */
		if (clnt4)
			esunrpc_rpc_shutdown_client(clnt4);
		if (clnt)
				...
				const struct sockaddr_un *addr)
{
	 ...
	 */
	clnt = esunrpc_rpc_create(&args);
	if (IS_ERR(clnt)) {
		result = PTR_ERR(clnt);
		goto out;
	}

	clnt4 = esunrpc_rpc_bind_new_program(clnt, &rpcb_program, RPCBVERS_4);
	if (IS_ERR(clnt4))
		clnt4 = NULL;
				    ...
				    const struct rpc_timeout *timeo)
{
	...
	return esunrpc_rpc_create(&args);
}
...
static int rpcb_register_call(struct sunrpc_net *sn, struct rpc_clnt *clnt, struct rpc_message *msg, bool is_set)
{
	...
	error = esunrpc_rpc_call_sync(clnt, msg, flags);
	if (error < 0)
		return error;
...
/**
 * esunrpc_rpcb_getport_async - obtain the port for a given RPC service on a given host
 * @task: task that is waiting for portmapper request
 *
 * This one can be called for an ongoing RPC request, and can be used in
 * an async (rpciod) context.
 */
void esunrpc_rpcb_getport_async(struct rpc_task *task)
{
	...
	rcu_read_unlock();
	xprt = esunrpc_xprt_get(task->tk_xprt);

	/* Put self on the wait queue to ensure we get notified if
	 * some other task is already attempting to bind the port */
	esunrpc_rpc_sleep_on_timeout(&xprt->binding, task,
	...
	salen = esunrpc_rpc_peeraddr(clnt, sap, sizeof(addr));
	...
	map = kzalloc_obj(struct rpcbind_args, esunrpc_rpc_task_gfp_mask());
	if (!map) {
		status = -ENOMEM;
	...
	case RPCBVERS_3:
		...
		map->r_addr = rpc_sockaddr2uaddr(sap, esunrpc_rpc_task_gfp_mask());
		if (!map->r_addr) {
			status = -ENOMEM;
	...
	esunrpc_rpc_release_client(rpcb_clnt);
	if (IS_ERR(child)) {
...
EXPORT_SYMBOL_GPL(esunrpc_rpcb_getport_async);
			     ...
			     const void *data)
{
	...
	p = esunrpc_xdr_reserve_space(xdr, RPCB_mappingargs_sz << 2);
			    ...
			    void *data)
{
	...
	p = esunrpc_xdr_inline_decode(xdr, 4);
	if (unlikely(p == NULL))
		return -EIO;
			       ...
			       const u32 maxstrlen)
{
	...
	if (len > maxstrlen)
		...
		len = maxstrlen;
	p = esunrpc_xdr_reserve_space(xdr, 4 + len);
	esunrpc_xdr_encode_opaque(p, string, len);
}
			     ...
			     const void *data)
{
	...
	p = esunrpc_xdr_reserve_space(xdr, (RPCB_program_sz + RPCB_version_sz) << 2);
	*p++ = cpu_to_be32(rpcb->r_prog);
	*p = cpu_to_be32(rpcb->r_vers);
			    ...
			    void *data)
{
	...
	if (esunrpc_rpc_uaddr2sockaddr(req->rq_xprt->xprt_net, (char *)p, len,
