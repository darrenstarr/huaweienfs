...
int esunrpc_rpc_pipefs_notifier_register(struct notifier_block *nb)
{
	return blocking_notifier_chain_register(&rpc_pipefs_notifier_list, nb);
}
EXPORT_SYMBOL_GPL(esunrpc_rpc_pipefs_notifier_register);

void esunrpc_rpc_pipefs_notifier_unregister(struct notifier_block *nb)
{
	blocking_notifier_chain_unregister(&rpc_pipefs_notifier_list, nb);
}
EXPORT_SYMBOL_GPL(esunrpc_rpc_pipefs_notifier_unregister);
...
ssize_t esunrpc_rpc_pipe_generic_upcall(struct file *filp, struct rpc_pipe_msg *msg,
...
EXPORT_SYMBOL_GPL(esunrpc_rpc_pipe_generic_upcall);

/**
 * esunrpc_rpc_queue_upcall - queue an upcall message to userspace
 ...
 */
int
esunrpc_rpc_queue_upcall(struct rpc_pipe *pipe, struct rpc_pipe_msg *msg)
{
	int res = -EPIPE;
	struct dentry *dentry;
...
EXPORT_SYMBOL_GPL(esunrpc_rpc_queue_upcall);
...
rpc_show_info(struct seq_file *m, void *v)
{
	...
	seq_printf(m, "protocol: %s\n", esunrpc_rpc_peeraddr2str(clnt, RPC_DISPLAY_PROTO));
	seq_printf(m, "port: %s\n", esunrpc_rpc_peeraddr2str(clnt, RPC_DISPLAY_PORT));
	rcu_read_unlock();
	return 0;
}
...
void esunrpc_rpc_destroy_pipe_data(struct rpc_pipe *pipe)
{
	kfree(pipe);
}
EXPORT_SYMBOL_GPL(esunrpc_rpc_destroy_pipe_data);

struct rpc_pipe *esunrpc_rpc_mkpipe_data(const struct rpc_pipe_ops *ops, int flags)
{
	struct rpc_pipe *pipe;
...
EXPORT_SYMBOL_GPL(esunrpc_rpc_mkpipe_data);
...
/**
 * esunrpc_rpc_mkpipe_dentry - make an rpc_pipefs file for kernel<->userspace
 *		       communication
 ...
 * esunrpc_rpc_queue_upcall().  The actual reads will result in calls to
 ...
 */
int esunrpc_rpc_mkpipe_dentry(struct dentry *parent, const char *name,
...
EXPORT_SYMBOL_GPL(esunrpc_rpc_mkpipe_dentry);

/**
 * esunrpc_rpc_unlink - remove a pipe
 ...
 */
void
esunrpc_rpc_unlink(struct rpc_pipe *pipe)
{
	if (pipe->dentry) {
...
EXPORT_SYMBOL_GPL(esunrpc_rpc_unlink);

/**
 * esunrpc_rpc_init_pipe_dir_head - initialise a struct rpc_pipe_dir_head
 * @pdh: pointer to struct rpc_pipe_dir_head
 */
void esunrpc_rpc_init_pipe_dir_head(struct rpc_pipe_dir_head *pdh)
{
	INIT_LIST_HEAD(&pdh->pdh_entries);
	pdh->pdh_dentry = NULL;
}
EXPORT_SYMBOL_GPL(esunrpc_rpc_init_pipe_dir_head);

/**
 * esunrpc_rpc_init_pipe_dir_object - initialise a struct rpc_pipe_dir_object
 ...
 */
void esunrpc_rpc_init_pipe_dir_object(struct rpc_pipe_dir_object *pdo,
...
EXPORT_SYMBOL_GPL(esunrpc_rpc_init_pipe_dir_object);
...
/**
 * esunrpc_rpc_add_pipe_dir_object - associate a rpc_pipe_dir_object to a directory
 ...
 */
int
esunrpc_rpc_add_pipe_dir_object(struct net *net,
		struct rpc_pipe_dir_head *pdh,
		struct rpc_pipe_dir_object *pdo)
{
	int ret = 0;
...
EXPORT_SYMBOL_GPL(esunrpc_rpc_add_pipe_dir_object);

/**
 * esunrpc_rpc_remove_pipe_dir_object - remove a rpc_pipe_dir_object from a directory
 * @net: pointer to struct net
 * @pdh: pointer to struct rpc_pipe_dir_head
 * @pdo: pointer to struct rpc_pipe_dir_object
 *
 */
void
esunrpc_rpc_remove_pipe_dir_object(struct net *net,
		struct rpc_pipe_dir_head *pdh,
		struct rpc_pipe_dir_object *pdo)
{
	if (!list_empty(&pdo->pdo_head)) {
...
EXPORT_SYMBOL_GPL(esunrpc_rpc_remove_pipe_dir_object);

/**
 * esunrpc_rpc_find_or_alloc_pipe_dir_object
 * @net: pointer to struct net
 * @pdh: pointer to struct rpc_pipe_dir_head
...
esunrpc_rpc_find_or_alloc_pipe_dir_object(struct net *net,
		struct rpc_pipe_dir_head *pdh,
...
EXPORT_SYMBOL_GPL(esunrpc_rpc_find_or_alloc_pipe_dir_object);
 ...
 */
struct dentry *esunrpc_rpc_d_lookup_sb(const struct super_block *sb,
			       const unsigned char *dir_name)
{
	return try_lookup_noperm(&QSTR(dir_name), sb->s_root);
}
EXPORT_SYMBOL_GPL(esunrpc_rpc_d_lookup_sb);

int rpc_pipefs_init_net(struct net *net)
{
	struct sunrpc_net *sn = net_generic(net, esunrpc_sunrpc_net_id);

	sn->gssd_dummy = esunrpc_rpc_mkpipe_data(&gssd_dummy_pipe_ops, 0);
...
void rpc_pipefs_exit_net(struct net *net)
{
	...
	esunrpc_rpc_destroy_pipe_data(sn->gssd_dummy);
}

/*
 * This call will be used for per network namespace operations calls.
 * Note: Function will be returned with pipefs_sb_lock taken if superblock was
 * found. This lock have to be released by esunrpc_rpc_put_sb_net() when all operations
 * will be completed.
 */
struct super_block *esunrpc_rpc_get_sb_net(const struct net *net)
{
	struct sunrpc_net *sn = net_generic(net, esunrpc_sunrpc_net_id);
...
EXPORT_SYMBOL_GPL(esunrpc_rpc_get_sb_net);

void esunrpc_rpc_put_sb_net(const struct net *net)
{
	struct sunrpc_net *sn = net_generic(net, esunrpc_sunrpc_net_id);
...
EXPORT_SYMBOL_GPL(esunrpc_rpc_put_sb_net);
...
static const struct rpc_pipe_ops gssd_dummy_pipe_ops = {
	.upcall		= esunrpc_rpc_pipe_generic_upcall,
...
rpc_gssd_dummy_populate(struct dentry *root, struct rpc_pipe *pipe_data)
{
	...
	if (!err)
		err = esunrpc_rpc_mkpipe_dentry(clnt_dentry, "gssd", NULL, pipe_data);
	return err;
}
...
bool
esunrpc_gssd_running(struct net *net)
{
	struct sunrpc_net *sn = net_generic(net, esunrpc_sunrpc_net_id);
...
EXPORT_SYMBOL_GPL(esunrpc_gssd_running);
...
MODULE_ALIAS_FS("esunrpc_pipefs");
MODULE_ALIAS("esunrpc_pipefs");
...
int register_rpc_pipefs(void)
{
	...
	rpc_inode_cachep = kmem_cache_create("esunrpc_inode_cache",
