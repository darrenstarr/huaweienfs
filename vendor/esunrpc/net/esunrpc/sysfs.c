...
rpc_sysfs_xprt_kobj_get_xprt(struct kobject *kobj)
{
	...
	return esunrpc_xprt_get(x->xprt);
}
					    ...
					    const char *buf, size_t count)
{
	...
	xprt->addrlen = esunrpc_rpc_pton(xprt->xprt_net, buf, buf_len, saddr,
				 sizeof(*saddr));
...
{
	rpc_sunrpc_kset = kset_create_and_add("esunrpc", NULL, kernel_kobj);
	if (!rpc_sunrpc_kset)
		return -ENOMEM;
