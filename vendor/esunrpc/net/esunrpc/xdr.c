 ...
 */
__be32 *
esunrpc_xdr_encode_netobj(__be32 *p, const struct xdr_netobj *obj)
{
...
EXPORT_SYMBOL_GPL(esunrpc_xdr_encode_netobj);

/**
 * esunrpc_xdr_encode_opaque_fixed - Encode fixed length opaque data
 ...
 *
 */
__be32 *esunrpc_xdr_encode_opaque_fixed(__be32 *p, const void *ptr, unsigned int nbytes)
{
	if (likely(nbytes != 0)) {
...
EXPORT_SYMBOL_GPL(esunrpc_xdr_encode_opaque_fixed);

/**
 * esunrpc_xdr_encode_opaque - Encode variable length opaque data
 * @p: pointer to current position in XDR buffer.
 * @ptr: pointer to data to encode (or NULL)
 * @nbytes: size of data.
 *
...
__be32 *esunrpc_xdr_encode_opaque(__be32 *p, const void *ptr, unsigned int nbytes)
{
	*p++ = cpu_to_be32(nbytes);
	return esunrpc_xdr_encode_opaque_fixed(p, ptr, nbytes);
}
EXPORT_SYMBOL_GPL(esunrpc_xdr_encode_opaque);

__be32 *
esunrpc_xdr_encode_string(__be32 *p, const char *string)
{
	return xdr_encode_array(p, string, strlen(string));
}
EXPORT_SYMBOL_GPL(esunrpc_xdr_encode_string);

/**
 * esunrpc_xdr_terminate_string - '\0'-terminate a string residing in an xdr_buf
 * @buf: XDR buffer where string resides
 * @len: length of string, in bytes
 *
 */
void esunrpc_xdr_terminate_string(const struct xdr_buf *buf, const u32 len)
{
	char *kaddr;
...
EXPORT_SYMBOL_GPL(esunrpc_xdr_terminate_string);
...
/**
 * esunrpc_xdr_buf_to_bvec - Copy components of an xdr_buf into a bio_vec array
 ...
 */
unsigned int esunrpc_xdr_buf_to_bvec(struct bio_vec *bvec, unsigned int bvec_size,
...
EXPORT_SYMBOL_GPL(esunrpc_xdr_buf_to_bvec);

/**
 * esunrpc_xdr_inline_pages - Prepare receive buffer for a large reply
 ...
 */
void
esunrpc_xdr_inline_pages(struct xdr_buf *xdr, unsigned int offset,
...
EXPORT_SYMBOL_GPL(esunrpc_xdr_inline_pages);
 ...
 */
void
esunrpc__copy_from_pages(char *p, struct page **pages, size_t pgbase, size_t len)
{
	struct page **pgfrom;
	char *vfrom;
	size_t copy;
...
EXPORT_SYMBOL_GPL(esunrpc__copy_from_pages);
				     ...
				     unsigned int shift)
{
	...
	esunrpc__copy_from_pages(tail->iov_base + tato, buf->pages,
				    ...
				    unsigned int shift)
{
	...
	if (shift > base) {
		...
		if (hdto + hdlen > head->iov_len)
			hdlen = head->iov_len - hdto;
		esunrpc__copy_from_pages(head->iov_base + hdto, buf->pages,
...
/**
 * esunrpc_xdr_stream_pos - Return the current offset from the start of the xdr_stream
 * @xdr: pointer to struct xdr_stream
 */
unsigned int esunrpc_xdr_stream_pos(const struct xdr_stream *xdr)
{
	return (unsigned int)(XDR_QUADLEN(xdr->buf->len) - xdr->nwords) << 2;
}
EXPORT_SYMBOL_GPL(esunrpc_xdr_stream_pos);
...
/**
 * esunrpc_xdr_page_pos - Return the current offset from the start of the xdr pages
 * @xdr: pointer to struct xdr_stream
 */
unsigned int esunrpc_xdr_page_pos(const struct xdr_stream *xdr)
{
	unsigned int pos = esunrpc_xdr_stream_pos(xdr);

	WARN_ON(pos < xdr->buf->head[0].iov_len);
	return pos - xdr->buf->head[0].iov_len;
}
EXPORT_SYMBOL_GPL(esunrpc_xdr_page_pos);

/**
 * esunrpc_xdr_init_encode - Initialize a struct xdr_stream for sending data.
 ...
 *	 length for us.
 */
void esunrpc_xdr_init_encode(struct xdr_stream *xdr, struct xdr_buf *buf, __be32 *p,
...
EXPORT_SYMBOL_GPL(esunrpc_xdr_init_encode);

/**
 * esunrpc_xdr_init_encode_pages - Initialize an xdr_stream for encoding into pages
 * @xdr: pointer to xdr_stream struct
 * @buf: pointer to XDR buffer into which to encode data
 *
 */
void esunrpc_xdr_init_encode_pages(struct xdr_stream *xdr, struct xdr_buf *buf)
{
	xdr_reset_scratch_buffer(xdr);
...
EXPORT_SYMBOL_GPL(esunrpc_xdr_init_encode_pages);

/**
 * esunrpc___xdr_commit_encode - Ensure all data is written to buffer
 ...
 * following esunrpc_xdr_reserve_space will do it.  But an explicit call may be
 * required at the end of encoding, or any other time when the xdr_buf
 * data might be read.
 */
void esunrpc___xdr_commit_encode(struct xdr_stream *xdr)
{
...
EXPORT_SYMBOL_GPL(esunrpc___xdr_commit_encode);
...
/**
 * esunrpc_xdr_reserve_space - Reserve buffer space for sending
 * @xdr: pointer to xdr_stream
 ...
 * esunrpc_xdr_reserve_space() or xdr_commit_encode() on @xdr. The current
 ...
 */
__be32 * esunrpc_xdr_reserve_space(struct xdr_stream *xdr, size_t nbytes)
{
	__be32 *p = xdr->p;
	__be32 *q;
...
EXPORT_SYMBOL_GPL(esunrpc_xdr_reserve_space);

/**
 * esunrpc_xdr_reserve_space_vec - Reserves a large amount of buffer space for sending
 * @xdr: pointer to xdr_stream
 * @nbytes: number of bytes to reserve
 *
 * The size argument passed to esunrpc_xdr_reserve_space() is determined based
 ...
 */
int esunrpc_xdr_reserve_space_vec(struct xdr_stream *xdr, size_t nbytes)
{
	size_t thislen;
	__be32 *p;
	...
	while (nbytes) {
		...
		p = esunrpc_xdr_reserve_space(xdr, thislen);
		if (!p)
			return -EMSGSIZE;
...
EXPORT_SYMBOL_GPL(esunrpc_xdr_reserve_space_vec);

/**
 * esunrpc_xdr_truncate_encode - truncate an encode buffer
 * @xdr: pointer to xdr_stream
 ...
 *
 */
void esunrpc_xdr_truncate_encode(struct xdr_stream *xdr, size_t len)
{
...
EXPORT_SYMBOL(esunrpc_xdr_truncate_encode);

/**
 * esunrpc_xdr_truncate_decode - Truncate a decoding stream
 * @xdr: pointer to struct xdr_stream
 * @len: Number of bytes to remove
 *
 */
void esunrpc_xdr_truncate_decode(struct xdr_stream *xdr, size_t len)
{
...
EXPORT_SYMBOL_GPL(esunrpc_xdr_truncate_decode);

/**
 * esunrpc_xdr_restrict_buflen - decrease available buffer space
 * @xdr: pointer to xdr_stream
 ...
 * of the buffer.
 */
int esunrpc_xdr_restrict_buflen(struct xdr_stream *xdr, int newbuflen)
{
	struct xdr_buf *buf = xdr->buf;
...
EXPORT_SYMBOL(esunrpc_xdr_restrict_buflen);

/**
 * esunrpc_xdr_write_pages - Insert a list of pages into an XDR buffer for sending
 * @xdr: pointer to xdr_stream
 ...
 * items into the tail.
 */
void esunrpc_xdr_write_pages(struct xdr_stream *xdr, struct page **pages, unsigned int base,
		 unsigned int len)
{
	struct xdr_buf *buf = xdr->buf;
	struct kvec *tail = buf->tail;
...
EXPORT_SYMBOL_GPL(esunrpc_xdr_write_pages);
...
/**
 * esunrpc_xdr_init_decode - Initialize an xdr_stream for decoding data.
 * @xdr: pointer to xdr_stream struct
 * @buf: pointer to XDR buffer from which to decode data
 * @p: current pointer inside XDR buffer
 * @rqst: pointer to controlling rpc_rqst, for debugging
 */
void esunrpc_xdr_init_decode(struct xdr_stream *xdr, struct xdr_buf *buf, __be32 *p,
		     struct rpc_rqst *rqst)
{
	xdr->buf = buf;
	xdr->page_kaddr = NULL;
...
EXPORT_SYMBOL_GPL(esunrpc_xdr_init_decode);

/**
 * esunrpc_xdr_init_decode_pages - Initialize an xdr_stream for decoding into pages
 * @xdr: pointer to xdr_stream struct
 * @buf: pointer to XDR buffer from which to decode data
 * @pages: list of pages to decode into
 * @len: length in bytes of buffer in pages
 */
void esunrpc_xdr_init_decode_pages(struct xdr_stream *xdr, struct xdr_buf *buf,
			   struct page **pages, unsigned int len)
{
	...
	buf->buflen =  len;
	buf->len = len;
	esunrpc_xdr_init_decode(xdr, buf, NULL, NULL);
}
EXPORT_SYMBOL_GPL(esunrpc_xdr_init_decode_pages);

/**
 * esunrpc_xdr_finish_decode - Clean up the xdr_stream after decoding data.
 * @xdr: pointer to xdr_stream struct
 */
void esunrpc_xdr_finish_decode(struct xdr_stream *xdr)
{
	xdr_stream_unmap_current_page(xdr);
}
EXPORT_SYMBOL(esunrpc_xdr_finish_decode);
...
/**
 * esunrpc_xdr_inline_decode - Retrieve XDR data to decode
 * @xdr: pointer to xdr_stream struct
 ...
 * pointer position.
 */
__be32 * esunrpc_xdr_inline_decode(struct xdr_stream *xdr, size_t nbytes)
{
	__be32 *p;
...
EXPORT_SYMBOL_GPL(esunrpc_xdr_inline_decode);

static void xdr_realign_pages(struct xdr_stream *xdr)
{
	struct xdr_buf *buf = xdr->buf;
	struct kvec *iov = buf->head;
	unsigned int cur = esunrpc_xdr_stream_pos(xdr);
	unsigned int copied;
...
/**
 * esunrpc_xdr_read_pages - align page-based XDR data to current pointer position
 * @xdr: pointer to xdr_stream struct
 ...
 */
unsigned int esunrpc_xdr_read_pages(struct xdr_stream *xdr, unsigned int len)
{
	unsigned int nwords = XDR_QUADLEN(len);
...
EXPORT_SYMBOL_GPL(esunrpc_xdr_read_pages);

/**
 * esunrpc_xdr_set_pagelen - Sets the length of the XDR pages
 * @xdr: pointer to xdr_stream struct
 ...
 */
void esunrpc_xdr_set_pagelen(struct xdr_stream *xdr, unsigned int len)
{
	struct xdr_buf *buf = xdr->buf;
	...
	if (len < buf->page_len) {
		...
		xdr_buf_head_shift_right(buf, esunrpc_xdr_stream_pos(xdr),
...
EXPORT_SYMBOL_GPL(esunrpc_xdr_set_pagelen);

/**
 * esunrpc_xdr_enter_page - decode data from the XDR page
 * @xdr: pointer to xdr_stream struct
 * @len: number of bytes of page data
 *
 * Moves data beyond the current pointer position from the XDR head[] buffer
 ...
 */
void esunrpc_xdr_enter_page(struct xdr_stream *xdr, unsigned int len)
{
...
EXPORT_SYMBOL_GPL(esunrpc_xdr_enter_page);

static const struct kvec empty_iov = {.iov_base = NULL, .iov_len = 0};

void esunrpc_xdr_buf_from_iov(const struct kvec *iov, struct xdr_buf *buf)
{
	buf->head[0] = *iov;
...
EXPORT_SYMBOL_GPL(esunrpc_xdr_buf_from_iov);

/**
 * esunrpc_xdr_buf_subsegment - set subbuf to a portion of buf
 * @buf: an xdr buffer
 ...
 */
int esunrpc_xdr_buf_subsegment(const struct xdr_buf *buf, struct xdr_buf *subbuf,
		       unsigned int base, unsigned int len)
{
...
EXPORT_SYMBOL_GPL(esunrpc_xdr_buf_subsegment);

/**
 * esunrpc_xdr_stream_subsegment - set @subbuf to a portion of @xdr
 ...
 */
bool esunrpc_xdr_stream_subsegment(struct xdr_stream *xdr, struct xdr_buf *subbuf,
			   unsigned int nbytes)
{
	unsigned int start = esunrpc_xdr_stream_pos(xdr);
	unsigned int remaining, len;

	/* Extract @subbuf and bounds-check the fn arguments */
	if (esunrpc_xdr_buf_subsegment(xdr->buf, subbuf, start, nbytes))
		return false;
...
EXPORT_SYMBOL_GPL(esunrpc_xdr_stream_subsegment);

/**
 * esunrpc_xdr_stream_move_subsegment - Move part of a stream to another position
 ...
 */
unsigned int esunrpc_xdr_stream_move_subsegment(struct xdr_stream *xdr, unsigned int offset,
					unsigned int target, unsigned int length)
{
	...
	if (offset < target) {
		shift = target - offset;
		if (esunrpc_xdr_buf_subsegment(xdr->buf, &buf, offset, shift + length) < 0)
			return 0;
		xdr_buf_head_shift_right(&buf, 0, length, shift);
	} else if (offset > target) {
		shift = offset - target;
		if (esunrpc_xdr_buf_subsegment(xdr->buf, &buf, target, shift + length) < 0)
			return 0;
		xdr_buf_head_shift_left(&buf, shift, length, shift);
	}
	return length;
}
EXPORT_SYMBOL_GPL(esunrpc_xdr_stream_move_subsegment);

/**
 * esunrpc_xdr_stream_zero - zero out a portion of an xdr_stream
 * @xdr: an xdr_stream to zero out
 * @offset: the starting point in the stream
 * @length: the number of bytes to zero
 */
unsigned int esunrpc_xdr_stream_zero(struct xdr_stream *xdr, unsigned int offset,
			     unsigned int length)
{
	struct xdr_buf buf;

	if (esunrpc_xdr_buf_subsegment(xdr->buf, &buf, offset, length) < 0)
		return 0;
...
EXPORT_SYMBOL_GPL(esunrpc_xdr_stream_zero);

/**
 * esunrpc_xdr_buf_trim - lop at most "len" bytes off the end of "buf"
 ...
 */
void esunrpc_xdr_buf_trim(struct xdr_buf *buf, unsigned int len)
{
	size_t cur;
...
EXPORT_SYMBOL_GPL(esunrpc_xdr_buf_trim);
				      ...
				      void *obj, unsigned int len)
{
	...
	esunrpc__copy_from_pages(obj, subbuf->pages, subbuf->page_base, this_len);
	len -= this_len;
	obj += this_len;
...
int esunrpc_read_bytes_from_xdr_buf(const struct xdr_buf *buf, unsigned int base,
			    void *obj, unsigned int len)
{
	struct xdr_buf subbuf;
	int status;

	status = esunrpc_xdr_buf_subsegment(buf, &subbuf, base, len);
	if (status != 0)
		return status;
	__read_bytes_from_xdr_buf(&subbuf, obj, len);
	return 0;
}
EXPORT_SYMBOL_GPL(esunrpc_read_bytes_from_xdr_buf);
...
/* obj is assumed to point to allocated memory of size at least len: */
int esunrpc_write_bytes_to_xdr_buf(const struct xdr_buf *buf, unsigned int base,
			   void *obj, unsigned int len)
{
	struct xdr_buf subbuf;
	int status;
...
EXPORT_SYMBOL_GPL(esunrpc_write_bytes_to_xdr_buf);

int esunrpc_xdr_decode_word(const struct xdr_buf *buf, unsigned int base, u32 *obj)
{
	__be32	raw;
	int	status;

	status = esunrpc_read_bytes_from_xdr_buf(buf, base, &raw, sizeof(*obj));
	if (status)
		return status;
	*obj = be32_to_cpu(raw);
	return 0;
}
EXPORT_SYMBOL_GPL(esunrpc_xdr_decode_word);

int esunrpc_xdr_encode_word(const struct xdr_buf *buf, unsigned int base, u32 obj)
{
	__be32	raw = cpu_to_be32(obj);

	return esunrpc_write_bytes_to_xdr_buf(buf, base, &raw, sizeof(obj));
}
EXPORT_SYMBOL_GPL(esunrpc_xdr_encode_word);
			    ...
			    struct xdr_array2_desc *desc, int encode)
{
	...
	if (encode) {
		if (esunrpc_xdr_encode_word(buf, base, desc->array_len) != 0)
			return -EINVAL;
	} else {
		if (esunrpc_xdr_decode_word(buf, base, &desc->array_len) != 0 ||
...
int esunrpc_xdr_decode_array2(const struct xdr_buf *buf, unsigned int base,
...
EXPORT_SYMBOL_GPL(esunrpc_xdr_decode_array2);

int esunrpc_xdr_encode_array2(const struct xdr_buf *buf, unsigned int base,
		      struct xdr_array2_desc *desc)
{
...
EXPORT_SYMBOL_GPL(esunrpc_xdr_encode_array2);

int esunrpc_xdr_process_buf(const struct xdr_buf *buf, unsigned int offset,
		    unsigned int len,
...
EXPORT_SYMBOL_GPL(esunrpc_xdr_process_buf);

/**
 * esunrpc_xdr_stream_decode_string_dup - Decode and duplicate variable length string
 * @xdr: pointer to xdr_stream
 ...
 */
ssize_t esunrpc_xdr_stream_decode_string_dup(struct xdr_stream *xdr, char **str,
...
EXPORT_SYMBOL_GPL(esunrpc_xdr_stream_decode_string_dup);

/**
 * esunrpc_xdr_stream_decode_opaque_auth - Decode struct opaque_auth (RFC5531 S8.2)
 * @xdr: pointer to xdr_stream
 ...
 */
ssize_t esunrpc_xdr_stream_decode_opaque_auth(struct xdr_stream *xdr, u32 *flavor,
...
EXPORT_SYMBOL_GPL(esunrpc_xdr_stream_decode_opaque_auth);

/**
 * esunrpc_xdr_stream_encode_opaque_auth - Encode struct opaque_auth (RFC5531 S8.2)
 * @xdr: pointer to xdr_stream
 ...
 */
ssize_t esunrpc_xdr_stream_encode_opaque_auth(struct xdr_stream *xdr, u32 flavor,
				      void *body, unsigned int body_len)
{
	ssize_t ret, len;
...
EXPORT_SYMBOL_GPL(esunrpc_xdr_stream_encode_opaque_auth);
