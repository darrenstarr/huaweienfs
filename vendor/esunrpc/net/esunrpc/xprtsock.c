...
#include <esunrpc/svcsock.h>
#include <esunrpc/xprtsock.h>
...
static void xs_format_common_peer_addresses(struct rpc_xprt *xprt)
{
	...
	case AF_LOCAL:
		...
		break;
	case AF_INET:
		(void)esunrpc_rpc_ntop(sap, buf, sizeof(buf));
		xprt->address_strings[RPC_DISPLAY_ADDR] =
						kstrdup(buf, GFP_KERNEL);
		sin = xs_addr_in(xprt);
...
xs_read_stream_reply(struct sock_xprt *transport, struct msghdr *msg, int flags)
{
	...
	req = esunrpc_xprt_lookup_rqst(xprt, transport->recv.xid);
	if (!req || (transport->recv.copied && !req->rq_private_buf.len)) {
		msg->msg_flags |= MSG_TRUNC;
		goto out;
	}
	esunrpc_xprt_pin_rqst(req);
	...
	spin_lock(&xprt->queue_lock);
	if (msg->msg_flags & (MSG_EOR|MSG_TRUNC))
		esunrpc_xprt_complete_rqst(req->rq_task, transport->recv.copied);
	else
		req->rq_private_buf.len = transport->recv.copied;
	esunrpc_xprt_unpin_rqst(req);
out:
	spin_unlock(&xprt->queue_lock);
	return ret;
}
...
static void xs_poll_check_readable(struct sock_xprt *transport)
{
	...
	if (!test_and_set_bit(XPRT_SOCK_DATA_READY, &transport->sock_state))
		queue_work(esunrpc_xprtiod_workqueue, &transport->recv_worker);
}
...
static void xs_stream_data_receive(struct sock_xprt *transport)
{
	...
	if (ret == -ESHUTDOWN)
		...
		esunrpc_xprt_wake_pending_tasks(&transport->xprt, -EACCES);
	else
...
static int xs_nospace(struct rpc_rqst *req, struct sock_xprt *transport)
{
	...
	if (xprt_connected(xprt)) {
		...
		sk->sk_write_pending++;
		esunrpc_xprt_wait_for_buffer_space(xprt);
	} else
		ret = -ENOTCONN;
...
{
	return xdr_alloc_bvec(buf, esunrpc_rpc_task_gfp_mask());
}
...
static int xs_udp_send_request(struct rpc_rqst *req)
{
	...
	if (!esunrpc_xprt_request_get_cong(xprt, req))
		return -EBADSLT;

	status = xdr_alloc_bvec(xdr, esunrpc_rpc_task_gfp_mask());
	if (status < 0)
		return status;
	req->rq_xtime = ktime_get();
...
static void xs_run_error_worker(struct sock_xprt *transport, unsigned int nr)
{
	...
	queue_work(esunrpc_xprtiod_workqueue, &transport->error_worker);
}
...
static void xs_reset_transport(struct sock_xprt *transport)
{
	...
	esunrpc_xprt_disconnect_done(xprt);
}
		...
		struct sk_buff *skb)
{
	...
	/* Look up and lock the request corresponding to the given XID */
	spin_lock(&xprt->queue_lock);
	rovr = esunrpc_xprt_lookup_rqst(xprt, *xp);
	if (!rovr)
		goto out_unlock;
	esunrpc_xprt_pin_rqst(rovr);
	esunrpc_xprt_update_rtt(rovr->rq_task);
	spin_unlock(&xprt->queue_lock);
	task = rovr->rq_task;
	...
	spin_lock(&xprt->transport_lock);
	esunrpc_xprt_adjust_cwnd(xprt, task, copied);
	spin_unlock(&xprt->transport_lock);
	spin_lock(&xprt->queue_lock);
	esunrpc_xprt_complete_rqst(task, copied);
	__UDPX_INC_STATS(sk, UDP_MIB_INDATAGRAMS);
out_unpin:
	esunrpc_xprt_unpin_rqst(rovr);
 out_unlock:
	spin_unlock(&xprt->queue_lock);
}
...
{
	spin_lock(&xprt->transport_lock);
	esunrpc_xprt_adjust_cwnd(xprt, task, -ETIMEDOUT);
	spin_unlock(&xprt->transport_lock);
}
...
out:
	xprt_clear_connecting(xprt);
	esunrpc_xprt_wake_pending_tasks(xprt, status);
	return status;
}
...
out:
	xprt_clear_connecting(xprt);
	esunrpc_xprt_unlock_connect(xprt, transport);
	esunrpc_xprt_wake_pending_tasks(xprt, status);
...
static void xs_tcp_setup_socket(struct work_struct *work)
{
	...
	if (test_and_clear_bit(XPRT_SOCK_CONNECT_SENT,
		...
		if (IS_ERR(sock)) {
			esunrpc_xprt_wake_pending_tasks(xprt, PTR_ERR(sock));
			goto out;
		}
	}
...
static int xs_tls_handshake_sync(struct rpc_xprt *lower_xprt, struct xprtsec_parms *xprtsec)
{
	...
	struct tls_handshake_args args = {
		...
		.ta_data	= esunrpc_xprt_get(lower_xprt),
...
static void xs_tcp_tls_setup_socket(struct work_struct *work)
{
	...
	lower_clnt = esunrpc_rpc_create(&args);
	if (IS_ERR(lower_clnt)) {
		trace_rpc_tls_unavailable(upper_clnt, upper_xprt);
		clear_bit(XPRT_SOCK_CONNECTING, &upper_transport->sock_state);
		xprt_clear_connecting(upper_xprt);
		esunrpc_xprt_wake_pending_tasks(upper_xprt, PTR_ERR(lower_clnt));
		xs_run_error_worker(upper_transport, XPRT_SOCK_WAKE_PENDING);
		goto out_unlock;
	}
	...
	esunrpc_rpc_shutdown_client(lower_clnt);
...
out_unlock:
	...
	esunrpc_xprt_unlock_connect(upper_xprt, upper_transport);
	return;
...
out_close:
	...
	/* esunrpc_xprt_force_disconnect() wakes tasks with a fixed tk_status code.
	 * Wake them first here to ensure they get our tk_status code.
	 */
	esunrpc_xprt_wake_pending_tasks(upper_xprt, status);
...
static void xs_connect(struct rpc_xprt *xprt, struct rpc_task *task)
{
	...
	WARN_ON_ONCE(!esunrpc_xprt_lock_connect(xprt, task, transport));

	if (transport->sock != NULL) {
		dprintk("RPC:       xs_connect delayed xprt %p for %lu "
			"seconds\n", xprt, xprt->reestablish_timeout / HZ);

		delay = esunrpc_xprt_reconnect_delay(xprt);
		esunrpc_xprt_reconnect_backoff(xprt, XS_TCP_INIT_REEST_TO);

	} else
		dprintk("RPC:       xs_connect scheduled xprt %p\n", xprt);

	transport->clnt = task->tk_client;
	queue_delayed_work(esunrpc_xprtiod_workqueue,
...
static void xs_wake_write(struct sock_xprt *transport)
{
	if (test_and_clear_bit(XPRT_SOCK_WAKE_WRITE, &transport->sock_state))
		esunrpc_xprt_write_space(&transport->xprt);
}
...
static void xs_wake_error(struct sock_xprt *transport)
{
	...
	if (sockerr < 0) {
		esunrpc_xprt_wake_pending_tasks(&transport->xprt, sockerr);
		xs_tcp_force_close(&transport->xprt);
	}
}
...
static void xs_wake_pending(struct sock_xprt *transport)
{
	if (test_and_clear_bit(XPRT_SOCK_WAKE_PENDING, &transport->sock_state))
		esunrpc_xprt_wake_pending_tasks(&transport->xprt, -EAGAIN);
}
...
static int bc_sendto(struct rpc_rqst *req)
{
	...
	req->rq_xtime = ktime_get();
	err = xdr_alloc_bvec(xdr, esunrpc_rpc_task_gfp_mask());
	if (err < 0)
		return err;
...
static const struct rpc_xprt_ops xs_local_ops = {
	.reserve_xprt		= esunrpc_xprt_reserve_xprt,
	.release_xprt		= esunrpc_xprt_release_xprt,
	.alloc_slot		= esunrpc_xprt_alloc_slot,
	.free_slot		= esunrpc_xprt_free_slot,
	...
	.wait_for_reply_request	= esunrpc_xprt_wait_for_reply_request_def,
	.close			= xs_close,
	.destroy		= xs_destroy,
...
static const struct rpc_xprt_ops xs_udp_ops = {
	...
	.reserve_xprt		= esunrpc_xprt_reserve_xprt_cong,
	.release_xprt		= esunrpc_xprt_release_xprt_cong,
	.alloc_slot		= esunrpc_xprt_alloc_slot,
	.free_slot		= esunrpc_xprt_free_slot,
	.rpcbind		= esunrpc_rpcb_getport_async,
	...
	.wait_for_reply_request	= esunrpc_xprt_wait_for_reply_request_rtt,
	.timer			= xs_udp_timer,
	.release_request	= esunrpc_xprt_release_rqst_cong,
	.close			= xs_close,
	.destroy		= xs_destroy,
				      ...
				      unsigned int max_slot_table_size)
{
	...
	xprt = esunrpc_xprt_alloc(args->net, sizeof(*new), slot_table_size,
			max_slot_table_size);
	if (xprt == NULL) {
...
int init_socket_xprt(void)
{
	...
	esunrpc_xprt_register_transport(&xs_local_transport);
	esunrpc_xprt_register_transport(&xs_udp_transport);
	esunrpc_xprt_register_transport(&xs_tcp_transport);
	esunrpc_xprt_register_transport(&xs_tcp_tls_transport);
	esunrpc_xprt_register_transport(&xs_bc_tcp_transport);
...
void cleanup_socket_xprt(void)
{
	...
	esunrpc_xprt_unregister_transport(&xs_local_transport);
	esunrpc_xprt_unregister_transport(&xs_udp_transport);
	esunrpc_xprt_unregister_transport(&xs_tcp_transport);
	esunrpc_xprt_unregister_transport(&xs_tcp_tls_transport);
	esunrpc_xprt_unregister_transport(&xs_bc_tcp_transport);
}
