...
struct cache_head *esunrpc_sunrpc_cache_lookup_rcu(struct cache_detail *detail,
...
EXPORT_SYMBOL_GPL(esunrpc_sunrpc_cache_lookup_rcu);
...
struct cache_head *esunrpc_sunrpc_cache_update(struct cache_detail *detail,
...
EXPORT_SYMBOL_GPL(esunrpc_sunrpc_cache_update);
...
static inline int cache_is_valid(struct cache_head *h)
{
	if (!test_bit(CACHE_VALID, &h->flags))
		...
		if (test_bit(CACHE_NEGATIVE, &h->flags))
			 ...
			 * esunrpc_sunrpc_cache_update, ensures that anyone
...
int esunrpc_cache_check_rcu(struct cache_detail *detail,
...
EXPORT_SYMBOL_GPL(esunrpc_cache_check_rcu);
 ...
 */
int esunrpc_cache_check(struct cache_detail *detail,
		struct cache_head *h, struct cache_req *rqstp)
{
	int rv;

	rv = esunrpc_cache_check_rcu(detail, h, rqstp);
	if (rv)
		cache_put(h, detail);
	return rv;
}
EXPORT_SYMBOL_GPL(esunrpc_cache_check);
...
void esunrpc_sunrpc_init_cache_detail(struct cache_detail *cd)
{
...
EXPORT_SYMBOL_GPL(esunrpc_sunrpc_init_cache_detail);

void esunrpc_sunrpc_destroy_cache_detail(struct cache_detail *cd)
{
	esunrpc_cache_purge(cd);
	spin_lock(&cache_list_lock);
...
EXPORT_SYMBOL_GPL(esunrpc_sunrpc_destroy_cache_detail);
...
EXPORT_SYMBOL_GPL(esunrpc_cache_flush);

void esunrpc_cache_purge(struct cache_detail *detail)
{
	struct cache_head *ch = NULL;
...
EXPORT_SYMBOL_GPL(esunrpc_cache_purge);
...
{
	return !esunrpc_fail_sunrpc.ignore_cache_wait &&
...
void esunrpc_qword_add(char **bpp, int *lp, char *str)
{
	char *bp = *bpp;
	int len = *lp;
	int ret;
...
EXPORT_SYMBOL_GPL(esunrpc_qword_add);

void esunrpc_qword_addhex(char **bpp, int *lp, char *buf, int blen)
{
	char *bp = *bpp;
	int len = *lp;
...
EXPORT_SYMBOL_GPL(esunrpc_qword_addhex);
...
int esunrpc_sunrpc_cache_pipe_upcall(struct cache_detail *detail, struct cache_head *h)
{
	if (test_and_set_bit(CACHE_PENDING, &h->flags))
		return 0;
	return cache_pipe_upcall(detail, h);
}
EXPORT_SYMBOL_GPL(esunrpc_sunrpc_cache_pipe_upcall);

int esunrpc_sunrpc_cache_pipe_upcall_timeout(struct cache_detail *detail,
				     struct cache_head *h)
{
	...
	return esunrpc_sunrpc_cache_pipe_upcall(detail, h);
}
EXPORT_SYMBOL_GPL(esunrpc_sunrpc_cache_pipe_upcall_timeout);
...
int esunrpc_qword_get(char **bpp, char *dest, int bufsize)
{
...
EXPORT_SYMBOL_GPL(esunrpc_qword_get);
...
void *esunrpc_cache_seq_start_rcu(struct seq_file *m, loff_t *pos)
	__acquires(RCU)
{
	rcu_read_lock();
	return __cache_seq_start(m, pos);
}
EXPORT_SYMBOL_GPL(esunrpc_cache_seq_start_rcu);

void *esunrpc_cache_seq_next_rcu(struct seq_file *file, void *p, loff_t *pos)
{
	return cache_seq_next(file, p, pos);
}
EXPORT_SYMBOL_GPL(esunrpc_cache_seq_next_rcu);

void esunrpc_cache_seq_stop_rcu(struct seq_file *m, void *p)
	__releases(RCU)
{
	rcu_read_unlock();
}
EXPORT_SYMBOL_GPL(esunrpc_cache_seq_stop_rcu);
...
static int c_show(struct seq_file *m, void *p)
{
	...
	if (esunrpc_cache_check_rcu(cd, cp, NULL))
		seq_puts(m, "# ");
...
static const struct seq_operations cache_content_op = {
	.start	= esunrpc_cache_seq_start_rcu,
	.next	= esunrpc_cache_seq_next_rcu,
	.stop	= esunrpc_cache_seq_stop_rcu,
	.show	= c_show,
};
			   ...
			   struct cache_detail *cd)
{
	...
	/* Always flush everything, so behave like esunrpc_cache_purge()
...
static int create_cache_proc_entries(struct cache_detail *cd, struct net *net)
{
	...
	sn = net_generic(net, esunrpc_sunrpc_net_id);
...
int esunrpc_cache_register_net(struct cache_detail *cd, struct net *net)
{
	int ret;

	esunrpc_sunrpc_init_cache_detail(cd);
	ret = create_cache_proc_entries(cd, net);
	if (ret)
		esunrpc_sunrpc_destroy_cache_detail(cd);
	return ret;
}
EXPORT_SYMBOL_GPL(esunrpc_cache_register_net);

void esunrpc_cache_unregister_net(struct cache_detail *cd, struct net *net)
{
	remove_cache_proc_entries(cd);
	esunrpc_sunrpc_destroy_cache_detail(cd);
}
EXPORT_SYMBOL_GPL(esunrpc_cache_unregister_net);

struct cache_detail *esunrpc_cache_create_net(const struct cache_detail *tmpl, struct net *net)
{
	struct cache_detail *cd;
	int i;
...
EXPORT_SYMBOL_GPL(esunrpc_cache_create_net);

void esunrpc_cache_destroy_net(struct cache_detail *cd, struct net *net)
{
	kfree(cd->hash_table);
	kfree(cd);
}
EXPORT_SYMBOL_GPL(esunrpc_cache_destroy_net);
...
int esunrpc_sunrpc_cache_register_pipefs(struct dentry *parent,
...
EXPORT_SYMBOL_GPL(esunrpc_sunrpc_cache_register_pipefs);

void esunrpc_sunrpc_cache_unregister_pipefs(struct cache_detail *cd)
{
	if (cd->pipefs) {
...
EXPORT_SYMBOL_GPL(esunrpc_sunrpc_cache_unregister_pipefs);

void esunrpc_sunrpc_cache_unhash(struct cache_detail *cd, struct cache_head *h)
{
	spin_lock(&cd->hash_lock);
...
EXPORT_SYMBOL_GPL(esunrpc_sunrpc_cache_unhash);
