...
tasks_show(struct seq_file *f, void *v)
{
	...
	seq_printf(f, "%5u %04x %6d 0x%x 0x%x %8ld %ps %sv%u %s a:%ps q:%s\n",
		...
		clnt->cl_clid, xid, esunrpc_rpc_task_timeout(task), task->tk_ops,
...
rpc_clnt_debugfs_register(struct rpc_clnt *clnt)
{
	...
	esunrpc_rpc_clnt_iterate_for_each_xprt(clnt, do_xprt_debugfs, &xprtnum);
}
...
struct fail_sunrpc_attr esunrpc_fail_sunrpc = {
	.attr			= FAULT_ATTR_INITIALIZER,
};
EXPORT_SYMBOL_GPL(esunrpc_fail_sunrpc);

static void fail_sunrpc_init(void)
{
	struct dentry *dir;

	dir = fault_create_debugfs_attr("esunrpc_fail_sunrpc", NULL,
					&esunrpc_fail_sunrpc.attr);

	debugfs_create_bool("ignore-client-disconnect", S_IFREG | 0600, dir,
			    &esunrpc_fail_sunrpc.ignore_client_disconnect);

	debugfs_create_bool("ignore-server-disconnect", S_IFREG | 0600, dir,
			    &esunrpc_fail_sunrpc.ignore_server_disconnect);

	debugfs_create_bool("ignore-cache-wait", S_IFREG | 0600, dir,
			    &esunrpc_fail_sunrpc.ignore_cache_wait);
}
#else
static void fail_sunrpc_init(void)
{
...
{
	topdir = debugfs_create_dir("esunrpc", NULL);

	rpc_clnt_dir = debugfs_create_dir("esunrpc_clnt", topdir);

	rpc_xprt_dir = debugfs_create_dir("esunrpc_xprt", topdir);
