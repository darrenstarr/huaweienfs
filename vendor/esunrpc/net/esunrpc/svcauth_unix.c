...
struct auth_domain *esunrpc_unix_domain_find(char *name)
{
...
EXPORT_SYMBOL_GPL(esunrpc_unix_domain_find);
...
{
	return esunrpc_sunrpc_cache_pipe_upcall(cd, h);
}
				  ...
				  char **bpp, int *blen)
{
	...
	esunrpc_qword_add(bpp, blen, im->m_class);
	esunrpc_qword_add(bpp, blen, text_addr);
	(*bpp)[-1] = '\n';
}
			  ...
			  char *mesg, int mlen)
{
	...
	/* class */
	len = esunrpc_qword_get(&mesg, class, sizeof(class));
	if (len <= 0) return -EINVAL;

	/* ip address */
	len = esunrpc_qword_get(&mesg, buf, mlen);
	if (len <= 0) return -EINVAL;

	if (esunrpc_rpc_pton(cd->net, buf, len, &address.sa, sizeof(address)) == 0)
		return -EINVAL;
	...
	if (len) {
		dom = esunrpc_unix_domain_find(buf);
		if (dom == NULL)
			return -ENOENT;
	} else
		dom = NULL;
		...
		struct in6_addr *addr)
{
	...
	ip.m_addr = *addr;
	ch = esunrpc_sunrpc_cache_lookup_rcu(cd, &ip.h,
		...
		struct unix_domain *udom, time64_t expiry)
{
	...
	ch = esunrpc_sunrpc_cache_update(cd, &ip.h, &ipm->h,
...
void esunrpc_svcauth_unix_purge(struct net *net)
{
	struct sunrpc_net *sn;

	sn = net_generic(net, esunrpc_sunrpc_net_id);
	esunrpc_cache_purge(sn->ip_map_cache);
}
EXPORT_SYMBOL_GPL(esunrpc_svcauth_unix_purge);
...
ip_map_cached_get(struct svc_xprt *xprt)
{
	...
	if (test_bit(XPT_CACHE_AUTH, &xprt->xpt_flags)) {
		...
		if (ipm != NULL) {
			sn = net_generic(xprt->xpt_net, esunrpc_sunrpc_net_id);
...
svcauth_unix_info_release(struct svc_xprt *xpt)
{
	...
	if (ipm != NULL) {
		...
		sn = net_generic(xpt->xpt_net, esunrpc_sunrpc_net_id);
		cache_put(&ipm->h, sn->ip_map_cache);
	}
}
...
{
	return esunrpc_sunrpc_cache_pipe_upcall_timeout(cd, h);
}
			     ...
			     char **bpp, int *blen)
{
	...
	esunrpc_qword_add(bpp, blen, tuid);
	(*bpp)[-1] = '\n';
}
			...
			char *mesg, int mlen)
{
	...
	if (ugp) {
		...
		ch = esunrpc_sunrpc_cache_update(cd,
					 &ug.h, &ugp->h,
					 unix_gid_hash(uid));
		if (!ch)
			err = -ENOMEM;
...
int unix_gid_cache_create(struct net *net)
{
	...
	cd = esunrpc_cache_create_net(&unix_gid_cache_template, net);
	if (IS_ERR(cd))
		return PTR_ERR(cd);
	err = esunrpc_cache_register_net(cd, net);
	if (err) {
		esunrpc_cache_destroy_net(cd, net);
		return err;
	}
...
void unix_gid_cache_destroy(struct net *net)
{
	...
	esunrpc_cache_unregister_net(cd, net);
	esunrpc_cache_destroy_net(cd, net);
}
...
static struct unix_gid *unix_gid_lookup(struct cache_detail *cd, kuid_t uid)
{
	...
	ug.uid = uid;
	ch = esunrpc_sunrpc_cache_lookup_rcu(cd, &ug.h, unix_gid_hash(uid));
	if (ch)
...
static struct group_info *unix_gid_find(kuid_t uid, struct svc_rqst *rqstp)
{
	...
	struct sunrpc_net *sn = net_generic(rqstp->rq_xprt->xpt_net,
					    esunrpc_sunrpc_net_id);

	ug = unix_gid_lookup(sn->unix_gid_cache, uid);
	if (!ug)
		return ERR_PTR(-EAGAIN);
	ret = esunrpc_cache_check(sn->unix_gid_cache, &ug->h, &rqstp->rq_chandle);
	switch (ret) {
	case -ENOENT:
...
esunrpc_svcauth_unix_set_client(struct svc_rqst *rqstp)
{
	...
	switch (esunrpc_cache_check(sn->ip_map_cache, &ipm->h, &rqstp->rq_chandle)) {
		default:
			BUG();
		case -ETIMEDOUT:
			return SVC_CLOSE;
...
EXPORT_SYMBOL_GPL(esunrpc_svcauth_unix_set_client);
...
svcauth_null_accept(struct svc_rqst *rqstp)
{
	...
	/* Call's verf field: */
	if (esunrpc_xdr_stream_decode_opaque_auth(xdr, &flavor, &body, &len) < 0)
		return SVC_GARBAGE;
	...
	if (esunrpc_xdr_stream_encode_opaque_auth(&rqstp->rq_res_stream,
...
struct auth_ops svcauth_null = {
	...
	.set_client	= esunrpc_svcauth_unix_set_client,
};
...
svcauth_tls_accept(struct svc_rqst *rqstp)
{
	...
	if (xprt->xpt_ops->xpo_handshake) {
		p = esunrpc_xdr_reserve_space(&rqstp->rq_res_stream, XDR_UNIT * 2 + 8);
		if (!p)
			return SVC_CLOSE;
...
svcauth_unix_accept(struct svc_rqst *rqstp)
{
	 ...
	 * fields.
	 */
	p = esunrpc_xdr_inline_decode(xdr, XDR_UNIT * 3);
	if (!p)
		return SVC_GARBAGE;
	len = be32_to_cpup(p + 2);
	if (len > RPC_MAX_MACHINENAME)
		return SVC_GARBAGE;
	if (!esunrpc_xdr_inline_decode(xdr, len))
		return SVC_GARBAGE;
	...
	if (len > UNX_NGROUPS)
		goto badcred;
	p = esunrpc_xdr_inline_decode(xdr, XDR_UNIT * len);
	if (!p)
		return SVC_GARBAGE;
...
int ip_map_cache_create(struct net *net)
{
	...
	cd = esunrpc_cache_create_net(&ip_map_cache_template, net);
	if (IS_ERR(cd))
		return PTR_ERR(cd);
	err = esunrpc_cache_register_net(cd, net);
	if (err) {
